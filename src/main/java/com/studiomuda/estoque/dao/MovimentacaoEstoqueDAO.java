...
import com.studiomuda.estoque.conexao.Conexao;
import com.studiomuda.estoque.config.ApplicationContextProvider;
import com.studiomuda.estoque.model.MovimentacaoEstoque;
import com.studiomuda.estoque.service.SuprimentoService;
...
public class MovimentacaoEstoqueDAO {
    public void registrar(MovimentacaoEstoque mov) throws SQLException {
        ...
        try (Connection conn = Conexao.getConnection();
            ...
            if (mov.getTipo().equalsIgnoreCase("saida")) {
                tryGerarRascunho(mov.getIdProduto());
            }
        } catch (SQLException e) {
            try (Connection conn = Conexao.getConnection()) {
                conn.rollback();
    ...
    private void tryGerarRascunho(int produtoId) {
        try {
            SuprimentoService suprimentoService = ApplicationContextProvider.getBean(SuprimentoService.class);
