...
public class SuprimentoService {
    ...
    @Transactional
    public void salvarParametro(int produtoId, String fornecedorNome, int leadTimeDias, int margemSeguranca) {
        validarParametro(produtoId, fornecedorNome, leadTimeDias, margemSeguranca);
    ...
    @Transactional(readOnly = true)
    public List<ParametroEstoque> listarParametrosComMetricas() {
    ...
    @Transactional
    public boolean gerarRascunhoSeNecessario(int produtoId) {
        ParametroEstoqueJpaEntity parametro = obterParametroRepository().buscarPorProduto(produtoId)
                .orElse(null);
        if (parametro == null) {
            return false;
    ...
    @Transactional
    public int gerarRascunhosPendentes() {
        int geradas = 0;
        for (ParametroEstoque parametro : listarParametrosComMetricas()) {
            if (parametro.isReposicaoNecessaria() && gerarRascunhoSeNecessario(parametro.getProdutoId())) {
                geradas++;
    ...
    @Transactional
    public void atualizarRascunho(int ordemId, int quantidade, double valorUnitario) {
    ...
    @Transactional
    public void aprovar(int ordemId) {
        alterarStatus(ordemId, OrdemCompra.STATUS_APROVADA);
    }

    @Transactional
    public void rejeitar(int ordemId) {
        alterarStatus(ordemId, OrdemCompra.STATUS_REJEITADA);
    }
    ...
    private void alterarStatus(int ordemId, String status) {
        OrdemCompraJpaEntity ordem = obterOrdemRepository().buscarCompletaPorId(ordemId)
                .orElseThrow(() -> new IllegalArgumentException("Ordem de compra nao encontrada."));
        if (!OrdemCompra.STATUS_RASCUNHO.equals(ordem.getStatus())) {
